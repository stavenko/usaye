use crate::{providers::types::TaskStatus, tests::app::create_app};
...
#[actix_web::test]
async fn add_task_immediately_runs() {
    let app = create_app().await;
    let payload = json! {{
        "task_url" : "http://example.com",
        "delay": "30m"
    ...
    let tv = result.payload.unwrap().pop().unwrap().task_status;
    assert_matches!(tv, TaskStatus::Running);

    assert_eq!(status, 200);
}

#[actix_web::test]
async fn fifths_task_is_pending() {
    let app = create_app().await;
    for _ in 0..5 {
        let payload = json! {{
            "task_url" : "http://example.com",
    ...
    println!("{:?}", text);
    let result = serde_json::from_slice::<Response<Vec<TaskView>, Error>>(&text).unwrap();
    let mut pl = result.payload.unwrap();
    for _ in 0..4 {
        let tv = pl.pop().unwrap().task_status;
        assert_matches!(tv, TaskStatus::Running);
    }

    let tv = pl.pop().unwrap().task_status;
    assert_matches!(tv, TaskStatus::Pending);
